...
def rand_str(length, prefix=''):
    return ''.join([prefix]+random.choices(string.ascii_lowercase + string.digits, k=length))
...
class WatcherTest(unittest.TestCase):
    ...
    test_results_dir = os.path.join(os.path.dirname(watcher_dir), 'test-results', 'config_watcher')
    ...
    def remove_resources(self, namespace='default'):
        v1 = client.CoreV1Api()
        cm_list = v1.list_namespaced_config_map(namespace, label_selector=f"{self.TEST_LABEL_KEY}")
        for cm in cm_list.items:
            v1.delete_namespaced_config_map(cm.metadata.name, cm.metadata.namespace)
        secrets_list = v1.list_namespaced_secret(namespace, label_selector=f"{self.TEST_LABEL_KEY}")
    ...
    def _get_configmap(self, name, namespace='default', data={}, extra_labels=None):
        metadata = client.V1ObjectMeta(
            name=name,
            namespace=namespace,
            labels=self._get_labels(extra_labels)
        )
    ...
    def awaitWebhookRequestCount(self, count, method='GET', path=None, payload=None, timeout=5):
        retry(lambda: self.assertWebhookRequestCount(count, method, path, payload), timeout)
    ...
    def testWatchMultipleNamespaces(self):
        ...
        try:
            ...
            data1 = dict(file1='content1', file2='content2')
            data2 = dict(file3='content3', file4='content4')
            cm1 = self._get_configmap(rand_str(6, 'cm-'), namespace=namespaces[0],
                                    data=data1)
            cm1 = v1.create_namespaced_config_map(namespaces[0], cm1)
            cm2 = self._get_configmap(rand_str(6, 'cm-'), namespace=namespaces[1],
                                    data=data2)
    ...
    def testWatchWithLabelSelector(self):
        ...
        v1 = client.CoreV1Api()
        cm1 = self._get_configmap(rand_str(6, 'cm-'),
                                 data=dict(file1='content1'))
        cm1 = v1.create_namespaced_config_map('default', cm1)
        cm2 = self._get_configmap(rand_str(6, 'cm-'),
                                 data=dict(file2='content2'),
                                 extra_labels=dict(foo='value1'))
        cm2 = v1.create_namespaced_config_map('default', cm2)
        cm3 = self._get_configmap(rand_str(6, 'cm-'),
                                 data=dict(file3='content3'),
                                 extra_labels=dict(foo='value2'))
        cm3 = v1.create_namespaced_config_map('default', cm3)
        cm4 = self._get_configmap(rand_str(6, 'cm-'),
    ...
    def _testWebhookInvoked(self, method='GET'):
        ...
        v1 = client.CoreV1Api()
        cm = self._get_configmap(rand_str(6, 'cm-'),
                                 data=dict(file1='content1', file2='content2'))
        cm = v1.create_namespaced_config_map('default', cm)
        # validate that webhook is called
        self.awaitWebhookRequestCount(1, method=method, path=webhook_path,
        ...
        self.awaitWebhookRequestCount(2, method=method, path=webhook_path,
                                      payload=webhook_payload)

        # delete the configMap
        v1.delete_namespaced_config_map(cm.metadata.name, cm.metadata.namespace)
        # validate that webhook is called one more time
        self.awaitWebhookRequestCount(3, method=method, path=webhook_path,
                                      payload=webhook_payload)
    ...
    def testNegativeWatcherConfig(self):
        self.start_watcher(env=dict(RESOURCE_TYPE='bogus'),
                           expect_error='Unsupported resource type: bogus')
        self.start_watcher(env=dict(WEBHOOK_URL='bogus'),
                           expect_error='Invalid webhook URL: bogus')
        self.start_watcher(env=dict(WEBHOOK_METHOD='bogus'),
                           expect_error='Invalid webhook method: bogus')
        self.start_watcher(env=dict(WEBHOOK_TIMEOUT='bogus'),
                           expect_error='Invalid webhook timeout: bogus')
        self.start_watcher(env=dict(SHUTDOWN_TIMEOUT='bogus'),
